...
from baseprop.cli.utils import LookupAction, Subcommand, build_data_from_files, parse_indices
...
def add_train_args(parser: ArgumentParser) -> ArgumentParser:
    ...
    mp_args.add_argument(
        "--hidden_channels", type=int, default=300, help="hidden dimension of the GCN"
    )
    mp_args.add_argument("--depth", type=int, default=3, help="Number of GCN layers.")
    mp_args.add_argument(
        "--dropout", type=float, default=0.0, help="dropout probability in GCN/FFN layers"
    )
    mp_args.add_argument(
        "--activation",
        type=uppercase,
    ...
    extra_mpnn_args.add_argument(
        "--no-batch-norm", action="store_true", help="Don't use batch normalization."
    )
    ...
    train_args.add_argument("--lr", type=float, default=1e-4, help="Initial learning rate.")
    train_args.add_argument(
...
def build_splits(args, format_kwargs, featurization_kwargs):
    ...
    all_data = build_data_from_files(args.data_path, **format_kwargs, **featurization_kwargs)
    ...
    for i_split in range(len(train_data)):
        sizes = [len(train_data[i_split]), len(val_data[i_split]), len(test_data[i_split])]
        logger.info(f"train/val/test split_{i_split} sizes: {sizes}")

    return train_data, val_data, test_data


def train_model(args, train_loader, val_loader, test_loader, output_dir, output_transform):
    for model_idx in range(args.ensemble_size):
        ...
        if test_loader is not None:
            ...
            evaluate_and_save_predictions(preds, test_loader, model.metrics, model_output_dir, args)
...
def evaluate_and_save_predictions(preds, test_loader, metrics, model_output_dir, args):
    ...
    for metric in metrics:
        ...
        for i, col in enumerate(args.target_columns):
            ...
            preds_loss = metric(preds_slice, targets_slice)
...
def main(args):
    ...
    for fold_idx, (train_data, val_data, test_data) in enumerate(zip(*splits)):
        ...
        train_model(args, train_loader, val_loader, test_loader, output_dir, output_transform)
