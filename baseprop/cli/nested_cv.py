...
from baseprop.cli.train import (TrainSubcommand, add_train_args, build_splits,
                                ...
                                save_smiles_splits, train_model)
...
def tune_model(args, data_splits, logger, monitor_mode, output_transform):
    ...
    ray_trainer = TorchTrainer(
        lambda config: train_with_splits(
            config,
            args,
            data_splits,
            logger,
...
def main(args: Namespace):
    ...
    for fold_idx in range(args.num_folds):
        ...
        outer_val_dset = MoleculeDataset(
            outer_val_data[fold_idx], atom_featurizer, bond_featurizer
        )
        ...
        if args.save_smiles_splits:
            save_smiles_splits(
                args, output_dir, outer_train_dset, outer_val_dset, outer_test_dset
            )
        ...
        results = tune_model(
            args, inner_data_splits, logger, monitor_mode, output_transform
        )
